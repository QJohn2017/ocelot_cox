...
def make_line(lattice_elements, cell_keys, BeamEnergy, BeamEnergy_ref):
    ...
    for key in lattice_elements['QuadLengths'].keys():
        LattObjs[key] = oclt.Quadrupole( \
                            ...
                            tilt = lattice_elements['QuadTilt'][key])
...
def beam_rotate(p_arrays,angl):
    for i in range(len(p_arrays)):

        x = p_arrays[i].rparticles[0,:]
        y = p_arrays[i].rparticles[2,:]
        x_new = x*np.cos(angl) - y*np.sin(angl)
        y_new = y*np.cos(angl) + x*np.sin(angl)
        p_arrays[i].rparticles[0,:] = x_new
        p_arrays[i].rparticles[2,:] = y_new

        x = p_arrays[i].rparticles[1,:]
        y = p_arrays[i].rparticles[3,:]
        x_new = x*np.cos(angl) - y*np.sin(angl)
        y_new = y*np.cos(angl) + x*np.sin(angl)
        p_arrays[i].rparticles[1,:] = x_new
        p_arrays[i].rparticles[3,:] = y_new
        
        x = p_arrays[i].x_c
        y = p_arrays[i].y_c
        x_new = x*np.cos(angl) - y*np.sin(angl)
        y_new = y*np.cos(angl) + x*np.sin(angl)
        p_arrays[i].x_c = x_new
        p_arrays[i].y_c = y_new

    return p_arrays

def beam_translate(p_arrays,dx=0,dy=0):
    for i in range(len(p_arrays)):
        p_arrays[i].rparticles[0,:] += dx 
        p_arrays[i].rparticles[2,:] += dy 
        p_arrays[i].x_c += dx
        p_arrays[i].y_c += dy
